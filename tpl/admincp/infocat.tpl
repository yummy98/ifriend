...
<!--{if $a eq "run"}-->
<div class="main-wrap">
  <div class="path"><p>当前位置：内容管理<span>&gt;&gt;</span>公告管理<span>&gt;&gt;</span><a href="<!--{$cpfile}-->?c=infocat">公告分类</a></p></div>
  <div class="main-cont">
    <h3 class="title"><a href="<!--{$cpfile}-->?c=infocat&a=add" class="btn-general"><span>添加分类</span></a>公告分类</h3>
	<form action="<!--{$cpfile}-->?c=infocat&a=del" method="post" name="myform" id="myform" style="margin:0">
    <table width="100%" border="0" cellpadding="0" cellspacing="0" class="table" align="center">
	  ...
	  <tr onMouseOver="overColor(this)" onMouseOut="outColor(this)">
		...
		<td align="center">
		<a href="<!--{$cpfile}-->?c=infocat&a=edit&id=<!--{$volist.catid}-->&page=<!--{$page}-->" class="icon-set">设置</a>&nbsp;&nbsp;<a href="<!--{$cpfile}-->?c=infocat&a=del&id[]=<!--{$volist.catid}-->" onClick="{if(confirm('确定要删除吗？一旦删除无法恢复！')){return true;} return false;}" class="icon-del">删除</a>
		</td>
	  </tr>
	  <!--{foreachelse}-->
      <tr>
...
<!--{if $a eq "add"}-->
<div class="main-wrap">
  <div class="path"><p>当前位置：内容管理<span>&gt;&gt;</span>公告管理<span>&gt;&gt;</span><a href="<!--{$cpfile}-->?c=infocat&a=add">添加公告分类</a></p></div>
  <div class="main-cont">
	<h3 class="title"><a href="<!--{$cpfile}-->?c=infocat" class="btn-general"><span>返回列表</span></a>添加公告分类</h3>
    <form name="myform" id="myform" method="post" action="<!--{$cpfile}-->?c=infocat" onsubmit='return checkform();' />
...
<div class="main-wrap">
  ...
  <div class="main-cont">
	<h3 class="title"><a href="<!--{$cpfile}-->?c=infocat&page=<!--{$page}-->" class="btn-general"><span>返回列表</span></a>编辑公告分类</h3>
    <form name="myform" id="myform" method="post" action="<!--{$cpfile}-->?c=infocat&page=<!--{$page}-->" onsubmit='return checkform();' />
	...
	<table cellpadding='1' cellspacing='1' class='tab'>
	  <tr>
		<td class='hback_1' width='15%'>分类名称 <span class="f_red">*</span> </td>
		<td class='hback' width='85%'>
		<input type="text" name="catname" id="catname" class="input-200" value="<!--{$infocat.catname}-->" />  <span id='dcatname' class='f_red'></span> 
		</td>
	  </tr>
	  <tr>
		<td class='hback_1'>排 序 <span class="f_red"></span> </td>
		<td class='hback'><input type="text" name="orders" id="orders" class="input-s" value="<!--{$infocat.orders}-->" />  <span id='dorders' class='f_red'></span> （数字越小越靠前）</td>
	  </tr>
	  <tr>
		<td class='hback_1'>打开方式 <span class="f_red"></span> </td>
		<td class='hback'>
		<select name="target" id="target">
		<option value="1"<!--{if $infocat.target=='1'}--> selected<!--{/if}-->>本页面</option>
		<option value="2"<!--{if $infocat.target=='2'}--> selected<!--{/if}-->>新页面</option>
		</select>
		<span id='dtarget' class='f_red'></span>
		</td>
	  </tr>
	  <tr>
		<td class='hback_1'>CSS样式 <span class="f_red"></span> </td>
		<td class='hback'><input type="text" name="cssname" id="cssname" class="input-100" value="<!--{$infocat.cssname}-->" />  <span id='dcssname' class='f_red'></span></td>
	  </tr>
	  <tr>
		<td class='hback_1'>分图标类 <span class="f_red"></span> </td>
		<td class='hback'>
		  <table border="0" cellspacing="0" cellpadding="0">
		    <tr>
			  <td><input type="text" name="img" id="img" class="input-txt" value="<!--{$infocat.img}-->" /> 
			  <span id="dimg"></span>
			  </td>
			  <td>
			  <iframe id='iframe_t' border='0' frameborder='0' scrolling='no' style="width:260px;height:25px;padding:0px;margin:0px;" src='<!--{$cpfile}-->?c=upload&formname=myform&module=article&uploadinput=img&multipart=sf_upload'></iframe>
	  ...
	  <tr>
		<td class='hback_1'>简 介 <span class="f_red"></span> </td>
		<td class='hback'><textarea name="intro" id="intro" style="width:50%;height:60px;overflow:auto;"><!--{$infocat.intro}--></textarea>  <span id='dintro' class='f_red'></span></td>
	  </tr>
	  <tr>
		<td class='hback_1'>Meta关键字 <span class="f_red"></span> </td>
		<td class='hback'><input type="text" name="metakeyword" id="metakeyword" class="input-txt" value="<!--{$infocat.metakeyword}-->" /> <span id='dmetakeyword' class='f_red'></span></td>
	  </tr>
	  <tr>
		<td class='hback_1'>Meta描述 <span class="f_red"></span> </td>
		<td class='hback'><textarea name="metadescription" id="metadescription" style="width:50%;height:60px;overflow:auto;"><!--{$infocat.metadescription}--></textarea>  <span id='dmetadescription' class='f_red'></span> </td>
	  </tr>
	  <tr>
		<td class='hback_1'>链接方式 <span class="f_red"></span> </td>
		<td class='hback'>
		<input type='radio' name='linktype' id='linktype' value='1'<!--{if $infocat.linktype=='1'}--> checked<!--{/if}--> />站内，
		<input type='radio' name='linktype' id='linktype' value='2'<!--{if $infocat.linktype=='2'}--> checked<!--{/if}--> />站外
		<span id='dlinketype' class='f_red'></span>
		</td>
	  </tr>
	  <tr>
		<td class='hback_1'>站外URL <span class="f_red"></span> </td>
		<td class='hback'><input type="text" name="linkurl" id="linkurl" class="input-txt" value="<!--{$infocat.linkurl}-->" />  <font color='#999999'>(如果是站外连接请填写URL)</font> <span id='dlinkurl' class='f_red'></span></td>
	  </tr>
	  <tr>
		<td class='hback_none'></td>
