...
$(function(){
	...
	//点击诚信查询 按钮
	$("[f='but_check_credit']").live("click", function(){
	...
	//查看举报详情
	$("[f='but_credit_details']").live("click", function(){
	...
	//关闭举报详情
	$("[f='but_close_credit_details']").live("click", function(){
...
function _oeCreditUser() {
	if (OECREDIT_HOME_UID > 0) {
		...
		$.ajax({
			type: "POST",
			url: _ROOT_PATH+"index.php?c=credit&a=getuser",
			cache: false,
...
function _oeCreditGet($type, $uid) {
	if ($type.length > 0 && $uid.length > 0) {
		...
		$.ajax({
			type: "POST",
			url: _ROOT_PATH+"index.php?c=credit&a=getcredit",
			cache: false,
...
function _oeCreditDetails($type, $uid) {
	if ($type.length > 0 && $uid.length > 0) {
		...
		$.ajax({
			type: "POST",
			url: _ROOT_PATH+"index.php?c=credit&a=getdetails",
			cache: false,
			data: {type:$type, uid:$uid, r:get_rndnum(8)},
			dataType: "json",
			success: function($data) {
