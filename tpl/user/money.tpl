...
<div class="user_main">
  <div class="oe_bn"><span></span><a href="<!--{$ucfile}-->">会员中心</a> &gt;&gt; 金币记录</div>
  ...
  <div class="main_right">
    ...
    <div class="div_smallnav_content_hover">
	  <!--{if $a eq 'run'}-->
	  <form method="post" id="search_form" action="<!--{$ucfile}-->?c=money" />
	  <div class="item" style="padding:10px 0px;">
		...
		<input type="submit" value="搜 索" name="btn_search" class="button-red" />		  
	  </div>
	  </form>
	  <table class="table">
		<tr> 	 	 	 	
		...
		<!--{foreach $moneylog as $volist}-->
		<tr>
		  <td><!--{$volist.timeline|date_format:"%Y-%m-%d %H:%M:%S"}--></td>
		  <td><!--{if $volist.actiontype==1}--><font color="green"><!--{$volist.amount}--></font><!--{/if}--></td>
		  <td><!--{if $volist.actiontype==2}--><font color="red"><!--{$volist.amount}--></font><!--{/if}--></td>
		  <td><font color="blue"><!--{$volist.remmoney}--></font></td>
