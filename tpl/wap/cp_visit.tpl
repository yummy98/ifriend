...
<div class="tab-layout-2">
  <ul>
    <li class="tab-selected" style="width:50%;">我看了谁</li>
    <li onclick="goUrl('<!--{$wapfile}-->?c=cp_visitme');" style="width:50%;">谁看了我</li>
  </ul>
  <div class="clear"></div>
</div>
<div class="cp-layout-body">
  ...
  <div class="cplist-box">
	...
	<ul id="json_data">
	  ...
	  <li onclick="userDetail('<!--{$volist.homeuserid}-->');">
  ...
  <div class="page-layout">
    页次：<!--{$page}-->/<!--{$pagecount}-->
	<!--{if $prepage>0}-->
	<span onclick="goUrl('<!--{$wapfile}-->?c=cp_visit&<!--{$urlitem}-->&page=<!--{$prepage}-->');">上一页</span>  
	<!--{/if}-->
	<!--{if $nextpage>0}-->
	<span onclick="goUrl('<!--{$wapfile}-->?c=cp_visit&<!--{$urlitem}-->&page=<!--{$nextpage}-->');">下一页</span>  
	<!--{/if}-->
  </div>
  <!--//ShowPage End-->
  <!--{/if}-->
